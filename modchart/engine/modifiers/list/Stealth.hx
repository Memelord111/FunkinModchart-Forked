...
class Stealth extends Modifier {
	...
	override public function visuals(data:VisualParameters, params:ModifierParameters) {
		...
		final visibility = params.isTapArrow ? getUnsafe(_stealthID, player) + getUnsafe(_stealthLaneIDs[lane], player) : getUnsafe(_darkID, player)
			+ getUnsafe(_darkLaneIDs[lane], player);
